...
| ~~M13~~ | **Settled 2026-09-16: yes.** `python -c "print(1)"` and `py -c "print(1)"` both printed `1` on the member's machine, while `python3` there is 20c's Store stub - which is exactly the shape `pick_python()` was written for, and the first confirmation that picking by *running* rather than by `command -v` is what that machine needs | - | Done |
...
| ~~M15~~ | **Answered 2026-09-16, incidentally: no.** With the cloud-drive folder as the working directory, `wsl.exe` could not translate it at all - it warned and started elsewhere. So WSL cannot map that drive, which is exactly the case the rejected design would have broken on and the shipped one never asks about | - | Done — and **the shipped design does not depend on the answer either way**: only bytes, exit codes and stderr cross the bridge (16g), never a path (`PRINCIPLES.md` invariant 11), so this would only matter if the rejected whole-environment bridge were ever revisited |
| M16 | Can the one-time code be typed into the Git Bash window itself (`winpty wsl.exe -e ssh -o ControlMaster=auto -o ControlPath=… -o ControlPersist=8h <host> true`)? | Run that line from Git Bash **in a real terminal window**. Attempted through an agent's own Bash tool on 2026-09-16 and it cannot be done there: `winpty` needs a tty and that tool has none, which is the same wall the one-time code itself is behind - a human turn cannot be taken by the thing waiting for it | **Yes** — this is the difference between never opening a WSL window and opening one once per work session |
| M17 | Does a real `powershell.exe` on the member's laptop answer `file_privacy()`'s query in the shape the parser expects - `me=`, `owner=`, `ace=` lines of SIDs? The parser was built from the documented output shape and tested against fixtures written by hand on Linux; no real Windows ACL has been through it | Run the one-liner `scripts/utils/portable.sh` builds (`_win_acl_script`) against the settings file in a Git Bash window and paste what comes back | **Yes** - until it is run once, "Windows says this file is private" is a claim the code makes on evidence it has never seen (PITFALLS 16k) |
